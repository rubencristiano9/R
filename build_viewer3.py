...
/* The releases picked in Strategy -> Custom, as dotted lines on the bar each one
   ...
   on. A release outside the loaded hours (08:30 data on an RTH tape) has no bar and
   is not drawn -- the Strategy panel's preview says how many. Cached like the time
   marks: it depends only on the view, the timeframe and the picks. */
    ...
    (NEWSCAL_BY_DATE[sn.day] || []).forEach(e => {
      ...
      for (let i = sn.a; i <= sn.b; i++)
        if (V.mins[i] <= e.etMinute && e.etMinute < V.mins[i] + S.tf){
          if (i >= a && i <= b) at.push([i, e.eventName + ' ' + hhmm(e.etMinute)]);
          return;
        }
    });
  });
  ...
  for (const date in NEWSCAL_BY_DATE){
    ...
    for (const e of chosen){
      ...
      if (stats && anchorDate >= lo && anchorDate <= hi){
        ...
        if (!S || i < 0){ stats.none++; if (stats.outEx === undefined) stats.outEx = plan; }
        else {
...
function newsPreviewHTML(C){
  ...
  /* the loaded hours, from the longest session (the first one may be a short day) */
  const full = sess.reduce((x, y) => (!x || y.b - y.a > x.b - x.a ? y : x), null);
  const hours = full ? hhmm(BASE.mins[full.a]) + '\u2013' + hhmm(BASE.mins[full.b]) : 'none';
  const dowOk = d => (C.cuDow || {})[DOW_NAMES[Core.dowOf(d)]];
  if (newsAround(C)){
    const st = {anchors: 0, moved: 0, none: 0, slots: {}, timedDays: {}};
    buildNewsSides(C, st);
