# ==============================================================================
# NQ + ES DIRECTION SEARCH — RICH FEATURES + XGBOOST DEPTH 2/3/4/5
#
# BIG-PICTURE GOAL
...
OUT <- file.path(
  dirname(NQ_FILE),
  "nq_es_rich_features_xgb_depth_2_3_4_5"
)
...
read_market <- function(file, prefix) {
  ...
  x <- x[ts >= as.POSIXct("2018-12-15 00:00:00", tz = "UTC")]
...
# ==============================================================================
# 7. RTH OPENING GAP + FIRST 30 MINUTES + PRIOR-DAY STATE
# ==============================================================================
...
RTH_DAILY <- D[
  ...
  .(
    ...
    nq_first30_close = nq_close[
      match(10L * 60L - 1L, bar_minute_et)
    ],
    es_first30_close = es_close[
      match(10L * 60L - 1L, bar_minute_et)
    ],
  ...
  by = .(
    rth_date = bar_date_et
  )
]

data.table::setorder(RTH_DAILY, rth_date)

# Incomplete opening windows are not used.
RTH_DAILY[
  first30_n < 30L,
  `:=`(
...
RTH_DAILY[, nq_prev_rth_ret := data.table::shift(
  log(nq_rth_close / nq_rth_open)
)]
RTH_DAILY[, es_prev_rth_ret := data.table::shift(
  log(es_rth_close / es_rth_open)
)]

RTH_DAILY[, nq_prev_rth_range := data.table::shift(
  (nq_rth_high - nq_rth_low) / nq_rth_open
)]
RTH_DAILY[, es_prev_rth_range := data.table::shift(
  (es_rth_high - es_rth_low) / es_rth_open
)]

# RTH opening gap = current 09:30 open versus previous RTH close.
RTH_DAILY[, nq_rth_gap := log(nq_rth_open / nq_prev_rth_close)]
RTH_DAILY[, es_rth_gap := log(es_rth_open / es_prev_rth_close)]

# "First 30 minutes green/red" = 09:30 open to 09:59 close, known at 10:00.
...
# First-30-minute volume relative to the PRIOR 20 RTH sessions.
RTH_DAILY[, nq_open30_vol_avg20_prior :=
  data.table::shift(
    as.numeric(TTR::SMA(nq_first30_volume, n = 20)),
    1L
  )]

RTH_DAILY[, es_open30_vol_avg20_prior :=
  data.table::shift(
    as.numeric(TTR::SMA(es_first30_volume, n = 20)),
    1L
  )]
...
# Merge by NY calendar date, then re-sort and restore row_id.
D <- merge(
  D,
  DAILY_FEATURES,
  by = "signal_date",
...
# Availability masks prevent future RTH information from leaking into pre-open rows.
D[, prev_rth_known := as.integer(
  !is.na(nq_prev_rth_ret)
)]

# 09:30 opening price belongs to the 09:30 bar; under our completed-bar rule,
# it is safely available from 09:31 onward. On the 15-minute signal grid,
# the first affected signal is 09:45.
D[, rth_gap_known := as.integer(
  !is.na(nq_rth_gap) &
  signal_minute >= (US_RTH_START_ET + 1L)
)]

# 09:30-09:59 is complete at exactly 10:00.
...
prev_cols <- c(
  "nq_prev_rth_ret", "nq_prev_rth_range",
  "es_prev_rth_ret", "es_prev_rth_range"
)

gap_cols <- c(
  "nq_rth_gap", "es_rth_gap"
)
...
for (v in prev_cols) {
  D[prev_rth_known == 0L | is.na(get(v)), (v) := 0]
}

for (v in gap_cols) {
  D[rth_gap_known == 0L | is.na(get(v)), (v) := 0]
}

for (v in open30_cols) {
  D[opening30_known == 0L | is.na(get(v)), (v) := 0]
}
...
rm(D, SIG, lookup_ts, lookup_close, lookup_row, DAILY_FEATURES, RTH_DAILY)
invisible(gc())
...
for (h in HORIZONS) {
  ...
  for (feature_set in names(FEATURE_SETS)) {
    ...
    for (model_id in MODEL_IDS) {
      ...
      model_key <- paste(h, feature_set, model_id, sep = "__")

      saved_models[[model_key]] <- list(
        model = model,
        features = features,
        horizon = h,
      ...
      for (target_coverage in COVERAGES) {
        ...
        selection_results[[result_counter]] <- cbind(
          data.table::data.table(
            horizon = h,
            feature_set = feature_set,
            model_id = model_id,
...
FROZEN <- saved_models[[champion_key]]
if (is.null(FROZEN)) stop("Frozen champion model not found.")
